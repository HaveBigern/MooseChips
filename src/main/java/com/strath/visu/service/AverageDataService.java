...
public class AverageDataService {
	...
	private Route createAverageDataClassList(List<Route> routes) {
		...
		for(List<DataClass> dataClassList : listOfLists) {			
			...
			avgDataClass.setBrakingCount(brakingCount/routes.size());
			avgDataClass.setCollisionOppDir(collisionOppDir/routes.size());
			avgDataClass.setCollisionTimeSameDir(collisionTimeSameDir/routes.size());
			avgDataClass.setCrashCount(crashCount/routes.size());
			avgDataClass.setDistanceOppDir(distanceOppDir/routes.size());
			avgDataClass.setDistanceSameDir(distanceSameDir/routes.size());
			avgDataClass.setDistanceTravelled(distanceTravelled/routes.size());
			avgDataClass.setGear(gear/routes.size());
			avgDataClass.setLateralAccel(lateralAccel/routes.size());
			avgDataClass.setLateralLanePosition(lateralLanePosition/routes.size());
			avgDataClass.setLateralVelocity(lateralVelocity/routes.size());
			avgDataClass.setLongAccelBrake(longAccelBrake/routes.size());
			avgDataClass.setLongAccelThrottle(longAccelThrottle/routes.size());
			avgDataClass.setLongitudeAccel(longitudeAccel/routes.size());
			avgDataClass.setLongitudeVelocity(longitudeVelocity/routes.size());
			avgDataClass.setOperatorMarker(operatorMarker/routes.size());
			avgDataClass.setRoadCurvature(roadCurvature/routes.size());
			avgDataClass.setSpeed(speed/routes.size());
			avgDataClass.setSpeedLimit(speedLimit/routes.size());
			avgDataClass.setSpeedLimitFtSec(speedLimitFtSec/routes.size());
			avgDataClass.setSteeringCount(steeringCount/routes.size());
			avgDataClass.setSteeringRate(steeringRate/routes.size());
			avgDataClass.setThrottleCount(throttleCount/routes.size());
			avgDataClass.setTimeElapsed(timeElapsed/routes.size());
